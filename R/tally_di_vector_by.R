#' Vector Tally of Di-Syllabic Words By Grouping Variable(s)
#'
#' Di-syllabic word tallies for the words in a vector of strings by grouping
#' variable(s).
#'
...
#' of di syllable words for each string in the vector.
...
#'    group = c("A", "B", "A")
#' )
#' tally_di_vector_by(dat$text, dat$group)
#'
#' with(presidential_debates_2012, tally_di_vector_by(dialogue, person))
#' with(presidential_debates_2012, tally_di_vector_by(dialogue, list(role, time)))
#' with(presidential_debates_2012, tally_di_vector_by(dialogue, list(person, time)))
tally_di_vector_by <- function(x, group, as.tibble = tibble_output(), ...){
    ...
    out <- long_dat[, list(n.words = length(stats::na.omit(count)),
        di = sum(count == 2, na.rm = TRUE)),  keyby = G]
